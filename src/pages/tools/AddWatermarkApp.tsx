...
export default function AddWatermarkApp() {
  ...
  const processApp = async () => {
    try {
      ...
      if(!files)return;
const wm = (document.getElementById('wm') || {value: 'WATERMARK'}).value||'WATERMARK';
const doc = await PDFDocument.load(await files[0].arrayBuffer(), { ignoreEncryption: true });
doc.getPages().forEach(p=>{
  const {width,height}=p.getSize();
  p.drawText(wm, {x:width/4, y:height/2, size:50, opacity:0.3, rotate:degrees(45), color:rgb(1,0,0)});
});
downloadBlob(await doc.save(), 'watermark.pdf', 'application/pdf'); toast.success('Done!');
    } catch (err) {
      console.error(err);
    ...
    <div className='max-w-4xl mx-auto p-4 md:p-8'>
      <Card className='shadow-lg border-primary/10'>
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          <div className='mt-4'><Label>Watermark Text</Label><Input id='wm' type='text' defaultValue='CONFIDENTIAL' /></div>
