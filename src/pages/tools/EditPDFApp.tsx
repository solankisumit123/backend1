...
export default function EditPDFApp() {
  ...
  const processApp = async () => {
    try {
      ...
      if(!files)return; const txt = (document.getElementById('txt') || {value: 'Added Note'}).value||'Added Note';
const doc = await PDFDocument.load(await files[0].arrayBuffer(), { ignoreEncryption: true });
doc.getPages()[0].drawText(txt, {x:50,y:50,size:20});
downloadBlob(await doc.save(), 'edited.pdf', 'application/pdf'); toast.success('Edited!');
    } catch (err) {
      console.error(err);
    ...
    <div className='max-w-4xl mx-auto p-4 md:p-8'>
      <Card className='shadow-lg border-primary/10'>
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          <div className='mt-4'><Label>Text to Append</Label><Input id='txt' type='text' /></div>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> Edit PDF
               </div>
            )}
          </Button>
        </CardContent>
