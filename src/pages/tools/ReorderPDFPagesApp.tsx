...
export default function ReorderPDFPagesApp() {
...
const list = pts.split(',').map(x=>parseInt(x.trim())-1);
for(let i of list){
  if(i>=0 && i<doc.getPageCount()){ const [p] = await newDoc.copyPages(doc,[i]); newDoc.addPage(p); }
}
downloadBlob(await newDoc.save(), 'reordered.pdf', 'application/pdf'); toast.success('Reordered!');
    } catch (err) {
      console.error(err);
    ...
    <div className='max-w-4xl mx-auto p-4 md:p-8'>
      <Card className='shadow-lg border-primary/10'>
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          <div className='mt-4'><Label>New Page Order (e.g. 3,1,2)</Label><Input id='pts' type='text' placeholder='3,1,2' /></div>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> Reorder PDF Pages
               </div>
            )}
          </Button>
        </CardContent>
