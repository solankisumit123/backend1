...
export default function DeletePDFPagesApp() {
  ...
  const processApp = async () => {
    try {
      ...
      if(!files)return;
const pts = (document.getElementById('pts') || {value: ''}).value||'';
const doc = await PDFDocument.load(await files[0].arrayBuffer(), { ignoreEncryption: true });
const list = pts.split(',').map(x=>parseInt(x.trim())-1).sort((a,b)=>b-a);
list.forEach(i=>{if(i>=0 && i<doc.getPageCount())doc.removePage(i)});
downloadBlob(await doc.save(), 'deleted.pdf', 'application/pdf'); toast.success('Deleted!');
    } catch (err) {
      console.error(err);
    ...
    <div className='max-w-4xl mx-auto p-4 md:p-8'>
      <Card className='shadow-lg border-primary/10'>
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          <div className='mt-4'><Label>Pages to Delete (comma separated, e.g. 1,3)</Label><Input id='pts' type='text' placeholder='1,2' /></div>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> Delete PDF Pages
               </div>
            )}
          </Button>
        </CardContent>
