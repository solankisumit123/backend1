...
export default function SignPDFApp() {
  ...
  const processApp = async () => {
    try {
      ...
      if(!files)return; const nm = (document.getElementById('nm') || {value: 'Signed'}).value||'Signed';
const doc = await PDFDocument.load(await files[0].arrayBuffer(), { ignoreEncryption: true });
const f = await doc.embedFont(StandardFonts.TimesRomanItalic);
doc.getPages()[0].drawText('Signed By: '+nm, {x:50,y:100,size:24,font:f});
downloadBlob(await doc.save(), 'signed.pdf', 'application/pdf'); toast.success('Signed!');
    } catch (err) {
      console.error(err);
    ...
    <div className='max-w-4xl mx-auto p-4 md:p-8'>
      <Card className='shadow-lg border-primary/10'>
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          <div className='mt-4'><Label>Signature Name</Label><Input id='nm' type='text' /></div>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> Sign PDF
               </div>
            )}
          </Button>
        </CardContent>
