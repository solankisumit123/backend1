...
export default function PPTtoPDFApp() {
...
setProgress(30); await new Promise(r=>setTimeout(r,1000));
const doc = await PDFDocument.create(); doc.addPage([1024,768]).drawText('PPT Slide via Tool', {x:100,y:400,size:30});
setProgress(90); downloadBlob(await doc.save(), 'slides.pdf', 'application/pdf'); toast.success('Mocked PPT to PDF completed');
    } catch (err) {
      console.error(err);
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> PPT to PDF
               </div>
            )}
          </Button>
        </CardContent>
