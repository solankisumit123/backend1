...
export default function PDFtoWordApp() {
...
for(let i=1;i<=pdf.numPages;i++){
  ...
  html += '<p>' + text.items.map(t=>t.str).join(' ') + '</p>';
}
html += '</body></html>';
downloadBlob(html, 'converted.doc', 'application/msword');
toast.success('Converted to Doc!');
    } catch (err) {
      console.error(err);
      ...
      <Card className='shadow-lg border-primary/10'>
        <CardHeader className='text-center space-y-2 bg-gradient-to-b from-primary/5 to-transparent pb-8'>
          <CardTitle className='text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-primary/80'>
            PDF to Word (Simulated Document)
          </CardTitle>
          <CardDescription className='text-lg'>
            Extract text to Word-compatible file.
          </CardDescription>
        </CardHeader>
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> PDF to Word (Simulated Document)
               </div>
            )}
          </Button>
        </CardContent>
