...
export default function JPGtoPDFApp() {
...
for(let i=0;i<files.length;i++){
  ...
  const img = await doc.embedJpg(await files[i].arrayBuffer());
  const {width, height} = img.scale(1);
  doc.addPage([width,height]).drawImage(img, {x:0,y:0,width,height});
}
setProgress(95); downloadBlob(await doc.save(), 'images.pdf', 'application/pdf'); toast.success('Converted!');
    } catch (err) {
      console.error(err);
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> JPG to PDF
               </div>
            )}
          </Button>
        </CardContent>
