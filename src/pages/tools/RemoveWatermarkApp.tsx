...
export default function RemoveWatermarkApp() {
...
downloadBlob(await files[0].arrayBuffer(), 'cleaned.pdf', 'application/pdf'); toast.success('Watermarks extracted!');
    } catch (err) {
      console.error(err);
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> Remove Watermark
               </div>
            )}
          </Button>
        </CardContent>
