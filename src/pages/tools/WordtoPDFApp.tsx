...
export default function WordtoPDFApp() {
  ...
  const processApp = async () => {
    try {
      ...
      if(!files||files.length===0)return toast.error('Upload file');
setProgress(30);
const txt = await files[0].text();
const doc = await PDFDocument.create();
const page = doc.addPage();
page.drawText(txt.substring(0,3000), {x:50, y:page.getSize().height-50, size:12, maxWidth:page.getSize().width-100});
setProgress(80);
downloadBlob(await doc.save(), 'doc.pdf', 'application/pdf');
toast.success('Saved to PDF');
    } catch (err) {
      console.error(err);
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> Word to PDF
               </div>
            )}
          </Button>
        </CardContent>
