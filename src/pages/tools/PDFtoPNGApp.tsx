...
export default function PDFtoPNGApp() {
  ...
  const processApp = async () => {
    try {
      ...
      if(!files)return;
const lib = await getPdfJs(); const zip = new JSZip();
const pdf = await lib.getDocument({data: await files[0].arrayBuffer()}).promise;
for(let i=1;i<=pdf.numPages;i++){
  ...
  zip.file('page_'+i+'.png', cv.toDataURL('image/png').split(',')[1], {base64:true});
}
setProgress(95); downloadBlob(await zip.generateAsync({type:'blob'}), 'pngs.zip', 'application/zip'); toast.success('Images Extracted!');
    } catch (err) {
      console.error(err);
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> PDF to PNG
               </div>
            )}
          </Button>
        </CardContent>
