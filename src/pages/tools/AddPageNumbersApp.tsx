...
export default function AddPageNumbersApp() {
...
doc.getPages().forEach((p,i)=>{
  ...
  p.drawText(String(i+1), {x:width/2, y:20, size:14});
});
downloadBlob(await doc.save(), 'numbered.pdf', 'application/pdf'); toast.success('Page numbers added!');
    } catch (err) {
      console.error(err);
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> Add Page Numbers
               </div>
            )}
          </Button>
        </CardContent>
