...
export default function RotatePDFApp() {
  ...
  const processApp = async () => {
    try {
      ...
      if(!files)return;
const r = parseInt((document.getElementById('rot') || {value: 90}).value)||90;
const doc = await PDFDocument.load(await files[0].arrayBuffer(), { ignoreEncryption: true });
doc.getPages().forEach(p=>p.setRotation(degrees(p.getRotation().angle+r)));
downloadBlob(await doc.save(), 'rotated.pdf', 'application/pdf'); toast.success('Rotated!');
    } catch (err) {
      console.error(err);
    ...
    <div className='max-w-4xl mx-auto p-4 md:p-8'>
      <Card className='shadow-lg border-primary/10'>
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          <div className='mt-4'><Label>Rotation Degrees (e.g. 90, 180)</Label><Input id='rot' type='number' defaultValue='90' step='90' /></div>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> Rotate PDF
               </div>
            )}
          </Button>
        </CardContent>
