...
export default function PDFtoTextApp() {
  ...
  const processApp = async () => {
    try {
      ...
      if(!files)return;
setProgress(20); const lib = await getPdfJs();
const pdf = await lib.getDocument({data: await files[0].arrayBuffer()}).promise;
let txt=''; for(let i=1;i<=pdf.numPages;i++){ setProgress(Math.round((i/pdf.numPages)*80));
  const p=await pdf.getPage(i); const tc=await p.getTextContent();
  txt+=tc.items.map(t=>t.str).join(' ')+'\n';
}
downloadBlob(txt, 'data.txt', 'text/plain'); toast.success('Extracted!');
    } catch (err) {
      console.error(err);
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> PDF to Text
               </div>
            )}
          </Button>
        </CardContent>
