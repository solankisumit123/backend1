...
export default function ExceltoPDFApp() {
  ...
  const processApp = async () => {
    try {
      ...
      if(!files||files.length===0)return toast.error('Upload CSV');
setProgress(30);
const doc = await PDFDocument.create();
const page = doc.addPage();
page.drawText((await files[0].text()).substring(0,3000), {x:50, y:page.getSize().height-50, size:10, maxWidth:page.getSize().width-100});
setProgress(80);
downloadBlob(await doc.save(), 'data.pdf', 'application/pdf');
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> Excel to PDF
               </div>
            )}
          </Button>
        </CardContent>
