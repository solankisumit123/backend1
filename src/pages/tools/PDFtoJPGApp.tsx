...
export default function PDFtoJPGApp() {
...
for(let i=1;i<=pdf.numPages;i++){
  ...
  zip.file('page_'+i+'.jpg', cv.toDataURL('image/jpeg', 0.9).split(',')[1], {base64:true});
}
setProgress(95); downloadBlob(await zip.generateAsync({type:'blob'}), 'jpgs.zip', 'application/zip');
        ...
        <CardContent className='p-6 md:p-12 space-y-8'>
          ...
          >
            {isProcessing ? (
               ...
               <div className='flex items-center gap-2'>
                 <Download className='h-5 w-5' /> PDF to JPG
               </div>
            )}
          </Button>
        </CardContent>
